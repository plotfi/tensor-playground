"""Triton solution for `hinge-loss` — mirrors solutions-cuda/hinge-loss.cu."""
import os
import sys
import torch
import triton
...
def _kernel(p_ptr, t_ptr, out_ptr, n, BLOCK: tl.constexpr):
    ...
    a = tl.maximum(0.0, 1 - p * t)
    h = tl.where(mask, a, 0.0)
...
def main(do_check):
    ...
    if do_check:
        d = (p - t).abs()
        ref = torch.clamp(1.0 - p * t, min=0.0).mean().view(1)
        return tb.check("hinge-loss", out, ref, rtol=1e-2, atol=1e-3)
    return 0


if __name__ == "__main__":
    tb.run("hinge-loss", main)
